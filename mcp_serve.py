...
Usage:
    ...
    hermes mcp serve --transport sse --port 8000
    hermes mcp serve --transport streamable-http --host 0.0.0.0 --port 8080

MCP client config (e.g. claude_desktop_config.json) — stdio:
    {
        "mcpServers": {
            "hermes": {
                "command": "hermes",
...
MCP client config (remote) — SSE/StreamableHTTP:
    {
        "mcpServers": {
            "hermes": {
...
def run_mcp_server(
    ...
    if transport not in ("stdio", "sse", "streamable-http"):
        print(
            ...
            f"Valid options: stdio, sse, streamable-http",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # For HTTP transports, configure host/port on the FastMCP settings object
